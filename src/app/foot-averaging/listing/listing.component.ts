...
@Component({
  selector: 'app-averaging-listing',
  templateUrl: './listing.component.html',
  styleUrls: ['./listing.component.scss']
})
export class AveragingListingComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.push(this.SearchService.search.subscribe((data) => {
      this.subscriptions.push((this.AveragingService.getSearchByPage(data.filter, data.page).subscribe({
        next: (response: any) => {
          this.format_filters()
  ...
  this.SpinnerService.show()
    this.userService.setPageSize(this.pagination.pageSize).subscribe({
      next: (data: any) => {
        ...
        this.AveragingService.getSearchByPage(this.SearchService.filters, 0).subscribe({
          next: (response: any) => {
            this.format_filters()
            this.SearchService.foot_list_changed.next(response)
            this.SpinnerService.hide()
  ...
  toggleCustomTable() {
    this.LocalService.open_custom_table.next({open:!this.tableCustom, isAverage: true})
  }
  delete() {
    this.AveragingService.deleteAverage(this.delete_ref).subscribe({
      next: (response: any) => {
       this.cancel()
  ...
  downloadExcel() {
    this.SpinnerService.show();
    this.subscriptions.push(this.StlService.getDownloadExcel("average").subscribe({
      next: (data) => {
        const name = 'FootEngineers_Foot_Average' + '.xlsx';
